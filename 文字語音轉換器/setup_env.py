...
def main():
    ...
    # 1. 檢查 Python 版本
    print("[1/5] 檢查 Python 系統環境...")
    print(f"  找到 Python 版本: {sys.version.split(' ')[0]}")
    print()

    # 2. 建立或檢查虛擬環境
    print("[2/5] 檢查/建立獨立的虛擬環境 (.venv)...")
    venv_dir = ".venv"
    ...
    # 3. 更新 pip 與核心工具
    print("[3/5] 更新 pip 核心套件安裝工具...")
    run_cmd([venv_python, "-m", "pip", "install", "--upgrade", "pip"], "更新 pip")
    print()

    # 4. 安裝需求套件
    print("[4/5] 開始下載並安裝 GPU 加速元件與相依模組...")
    print("  (下載 PyTorch 等大型套件可能會花費超過 5 分鐘，請保持網路通暢並勿關閉視窗)")
